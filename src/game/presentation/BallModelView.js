...
export function createBallModelView({ scenePort, document, worldX, worldZ, three = THREE_NAMESPACE } = {}) {
  ...
  const shadow = new THREE.Mesh(new THREE.CircleGeometry(.72, 32), shadowMaterial); shadow.name = "TonyBallContactShadow"; shadow.rotation.x = -Math.PI / 2; shadow.position.y = .018; shadow.renderOrder = 1; root.add(shadow);
  ...
  function render({ ball, selectedPlayer = null, selectedPlayerOwnsBall = false, activeCharge = null, ballStyle = "classic" } = {}) {
    ...
    mesh.position.set(0, .58 + motion.height + motion.meshVerticalOffset, 0);
    ...
    shadow.visible = motion.height < 7.2;
  ...
  function reset() {
    ...
    root.position.set(0, 0, 0); mesh.position.set(0, .58, 0); mesh.rotation.set(0, 0, 0); mesh.scale.set(1, 1, 1);
