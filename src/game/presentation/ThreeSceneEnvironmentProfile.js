...
export function createThreeSceneEnvironmentProfile(input = DEFAULT_PROFILE_INPUT) {
  ...
  finite(profile.renderer.fogDensity, "renderer.fogDensity");
  finite(profile.renderer.exposure, "renderer.exposure");
  positive(profile.camera.fov, "camera.fov");
  positive(profile.camera.lowPowerFov, "camera.lowPowerFov");
  ...
  for (const [styleName, style] of Object.entries(profile.pitchStyles)) {
    ...
    for (const key of ["exposure", "hemisphere", "flood", "rim", "stadium"]) finite(style.environment[key], `pitchStyles.${styleName}.environment.${key}`);
  }
