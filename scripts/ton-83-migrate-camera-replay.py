...
replace_once(
    ...
    // Replay phase and progress are engine snapshot facts. Presentation never advances them.
    return false;
  }''',
...
replace_once(
    ...
    '''  function renderFrame(alpha, now) {
    const frame = compatibilitySnapshots.createRenderFrame(alpha);
    cameraReplayBridge.project(Object.freeze({
    ...
    'render frame camera/replay projection',
)
replace_once(
...
if 'window.__TONY_CAMERA_REPLAY_BRIDGE__' not in text:
    raise RuntimeError("camera/replay bridge binding is missing")
