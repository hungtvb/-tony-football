...
const hairCorrectionSource = await readFile("src/game/presentation/PlayerV3HairCorrection.js", "utf8");
...
const bodyNode = character.json.nodes.find((node) => node.name === "SuperHero_Male"); const bodyPrimitive = character.json.meshes?.[bodyNode?.mesh]?.primitives?.[0]; const bodyPosition = character.json.accessors?.[bodyPrimitive?.attributes?.POSITION]; const bodyJoints = character.json.accessors?.[bodyPrimitive?.attributes?.JOINTS_0]; const bodyWeights = character.json.accessors?.[bodyPrimitive?.attributes?.WEIGHTS_0];
...
if (!bodyJoints || !bodyWeights || bodyJoints.count !== bodyPosition.count || bodyWeights.count !== bodyPosition.count) throw new Error(`${characterPath}: rig-weight kit requires JOINTS_0 and WEIGHTS_0 streams matching POSITION`);
...
for (const [sourceName, source, contracts] of [
  ...
  ["BrowserModelViewAdapter.js", adapterSource, ["DEFAULT_SIMULATION_SCALE_PROFILE", "worldUnitsPerSimulationUnit", "profileId", "createSnapshotRenderState", "createDefaultPlayerAssetLoader", "disposePlayerAssetTemplate", "repairPlayerV3Hair", "prepareModel", "kept procedural fallback", "appearanceDiagnostics(playerViews)", "integratedBodySurfaceCount", "bootRegionCount", "bootGeometryCount", "hairCoverageComplete", "measuredHeadBounds", "kitCoverageComplete", "hairlessPlayers", "distinctVariants", "visibleKitPlayers", "player-v3-integrated-body-material"]],

  ["RigFootballKitOverlay.js", rigAppearanceSource, ["TonyPlayerV3IntegratedAppearanceMaterial", "TonyRigFootballAppearanceV3", "BODY_VARIANTS", "KIT_WEIGHT_ATTRIBUTE", "rig-bone-weights-v2", "ensureRigKitWeights", "softWindow", "skinIndex", "skinWeight", "tonyKitWeights", "vTonyKitWeights", "tonyLayerThickness", "tonyPatternMix", "kitCoverageComplete", "leftBootVertices", "rightBootVertices", "tonyPlayerV3IntegratedAppearance", "tonyPlayerV3VariantIndex", "tonyPlayerV3VariantName", "tonyPlayerV3KitPattern", "player-v3-integrated-body-material", "TonyPlayerV3Hair", "TonyPlayerV3HairCrown", "tonyHairCoverageLayer", "hairCoverageComplete", "TonyFootballBootGeometry", "player-v3-skinned-footwear", "bootGeometryCount", "tonyRigHairGeometry", "tonySourceMapPreserved", "bootRegionCount", "rigidPrimitiveCount"]],
  ["PlayerV3HairCorrection.js", hairCorrectionSource, ["repairPlayerV3Hair", "body-rest-skinned-v1", "player-v3-skinned-hair", "TonyPlayerV3Hair", "TonyPlayerV3HairCrown", "skinGeometryToBone", "mesh.bind(body.skeleton, body.bindMatrix)", "tonyHairCoverageLayer"]],
  ...
  ["PlayerModelView.js", playerSource, ["new THREE.AnimationMixer(model)", "measureAndNormalizeRig", "representativeRigScale", "tonyScaleProfileId", "projectedHeight", "createSemanticPlayerMaterial", "classifyPlayerSurface", "tonySourceMapPreserved", "tonySharedTextures", "prepareModel", "player rig prepareModel must be a function", "disposeOwned(view.root, view.rig?.ownedMaterials)", "TonyBootLeft", "TonyBootRight", "applyFootballActionPose", "selectPlayerAnimationState", "stepPlayerMotionPresentation"]],

  ["BallModelView.js", ballSource, ["new THREE.SphereGeometry(BALL_RADIUS, 48, 32)", "DEFAULT_SIMULATION_SCALE_PROFILE", "createBallSurfaceTextures", "chargeRoot", "TonyBallContactShadow", "stepBallMotionPresentation", "shadowOpacity", "impactPulse"]],
...
for (const forbidden of ["TonyRigJersey", "TonyRigJerseyBand", "TonyRigShorts", "TonyRigSockLeft", "TonyRigSockRight", "TonyRigBootLeft", "TonyRigBootRight", "tonyRigBootGeometry", "bone.add(mesh)", "new THREE.BoxGeometry"]) if (rigAppearanceSource.includes(forbidden)) throw new Error(`RigFootballKitOverlay.js: rigid V2/block appearance primitive remains: ${forbidden}`);
for (const forbidden of ["bone.add(mesh)", "Head.attach", "head.attach", "new THREE.BoxGeometry", "MatchEngine", "dispatchEvent"]) if (hairCorrectionSource.includes(forbidden)) throw new Error(`PlayerV3HairCorrection.js: invalid hair placement/ownership remains: ${forbidden}`);
...
console.log(`Player V3 assets and presentation contracts valid: character ${(character.bytes.length / 1024).toFixed(0)} KB, animations ${(animation.bytes.length / 1024).toFixed(0)} KB, six variants, hybrid kit masks, body-rest skinned scalp coverage, skinned boot silhouettes, contact-synced motion, dynamic ball presentation, ${clipNames.size} clips.`);
