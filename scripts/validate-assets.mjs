...
if (!entrySource.includes('await import("./generated/game.js?v=25.0.0")')) throw new Error("browser-entry.js: missing TON-85 generated game entry import");
for (const contract of ["createBrowserModelViewAdapter", "createCanvasMatchRenderer", "createSnapshotCameraReplayAdapter", "cameraReplay: projection", "cameraReplayConsumer: owner", "createBrowserSettingsAdapter", "createBrowserEffectsAdapter", "createBrowserEffectsViewAdapter", "__TONY_COMPATIBILITY_PRESENTATION_PORT__", "direction: \"outward-only\"", "removalOwner: \"TON-65\""]) if (!entrySource.includes(contract)) throw new Error(`browser-entry.js: missing presentation contract ${contract}`);
