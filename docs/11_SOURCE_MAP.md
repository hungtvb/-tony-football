...
| `src/game/engine/` | R1 contracts, state, MatchEngine lifecycle, movement, ball simulation, kick actions, tackles, and teammate runs | Headless only; no DOM, Three.js, Canvas, audio, or render-frame dependencies |
...
| AI decisions and goalkeeper behavior | `game.js` | `game.js` | Existing gameplay and visual scenarios | Next Slice B checkpoint: AI produces deterministic decisions and engine-owned outcomes |
