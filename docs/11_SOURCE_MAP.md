...
## TON-85 final ownership map

| Surface | Runtime owner | Immutable input | CSS ownership |
| --- | --- | --- | --- |
| HUD and controls | `DomHudAdapter` | snapshot frame + input facts | `.hud-player-card`, control/status selectors |
| Radar | `RadarSnapshotAdapter` | authoritative snapshot | `#radarCanvas` |
| WebGL environment/models/effects | `BrowserThreeSceneEnvironmentHost`, `BrowserModelViewAdapter`, `BrowserEffectsViewAdapter` | shared frozen camera/replay/effects frame | `#gameCanvas`, `.screen-fx` |
| Canvas fallback | `CanvasMatchRenderer` | the same frozen camera/replay/effects frame | `#gameCanvas` |
| Event and preview audio | `BrowserAudioAdapter`, `BrowserSettingsAdapter` | typed events or user-preference commands | sound/settings controls |
| Temporary compatibility seam | `__TONY_COMPATIBILITY_PRESENTATION_PORT__` | copied/frozen outward facts only | none; removal owner TON-65 |
...
| `browser-entry.js` | Creates all browser presentation adapters, publishes the frozen outward-only compatibility port and imports the generated runtime | Sole browser composition root; no gameplay authority |
| `game.js` | Final compatibility composition and immutable snapshot/event mirrors | No HUD/radar/audio/WebGL/Canvas/camera/model/settings/effects owner; uses only the named outward port |
| `generated/game.js` | Deterministic normalized copy of tracked `game.js` for static build paths | Generated byte-equivalent runtime; never edited directly |
