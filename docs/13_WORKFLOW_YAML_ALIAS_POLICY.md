...
Tony Football workflow validation uses an explicit fail-closed rule for YAML anchors, aliases, and merge-key inheritance. Anchor names use the full non-flow token space, including numeric names such as `&1` and `*1`.
...
Workflow authors must expand permissions, steps, and jobs explicitly. Standard shorthand, secondary and verbatim YAML tags (`!custom`, `!!map`, `!<tag:…>`) fail closed, as do flow-style job mappings, step sequences and executable keys. Inline permission maps remain supported. The exception registry cannot suppress these rules.
...
- numeric anchor and alias names;
- shorthand, secondary and verbatim YAML tags;
...
- GitHub Script attempts to load modules/files through literal or constructed paths from a write-authorized job.
