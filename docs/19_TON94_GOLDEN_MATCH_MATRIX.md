...
| GM-05 | Representative controls | move, switch, pass, through ball, shot, tackle/press, sprint and possession/stat changes | required P1 lane starts from natural kickoff possession, records immutable browser-dispatched commands and observes authoritative possession/stat progression without a possession mutation seam | COVERED | P1 | input adapter / MatchEngine action routing |
...
| GM-07 | Replay isolation | first goal completes → later goal uses only its own incident | required browser lane drives both goals through browser input, recovers possession through public tackle/switch commands, and proves ordered incident-local frozen replay snapshots with scores [1,0] then [2,0] | COVERED | P1 | replay incident index / immutable projection |
...
| GM-09 | HUD, radar and statistics | score/clock/player/radar/stats remain coherent through actions, goal and restart | required P1 chronology asserts score, clock, stamina, radar repaint, possession, shots and pass UI against engine facts; goal and score cards are observed with bounded real wall-clock visibility | COVERED | P1 | HUD/radar adapters / immutable snapshots |
...
There are no open **reproduced** P0/P1 defects in the covered Phase-one lanes. The P1 continuation closes GM-04/05/07/09/11/12/13 with required executable evidence. The runtime harness is advance-only: it exposes no score or possession mutation seam. P2 GM-10 remains out of this bounded incident scope and must not be interpreted as permission to resume bridge deletion.
