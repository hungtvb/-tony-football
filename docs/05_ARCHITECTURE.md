...
- `ThreeSceneEnvironmentAdapter` owns WebGL availability, resize, context loss/restoration, explicit Canvas fallback signaling and scene-host lifecycle. Its host port exposes only object registration, immutable camera-pose operations, render requests and immutable diagnostics; raw renderer, scene and camera handles are not part of the contract.
