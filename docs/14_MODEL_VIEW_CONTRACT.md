...
Teardown invalidates every pending load generation and enters a terminating state that rejects attach, render, reset and asset installation. A player view removes its scene root, releases the current mixer and retries every deferred-retirement mixer. Any set whose `action.stop`, `stopAllAction` or `uncacheRoot` step still reports an error remains strongly owned and is retried by a later teardown call; neither the current rig reference nor the deferred collection may be cleared while cleanup is incomplete.

The browser model adapter retains every player view whose teardown reports an error and retries those views before tearing down the ball view or releasing the shared character template. Final root/material disposal occurs only after scene detachment and all current/deferred animation ownership has been released. Successful final disposal is recorded once, references are then cleared, and the view/adapter transitions from terminating to disposed. Late character or animation results are observed, disposed and ignored.

## Validation

TON-81 requires pure animation-state tests, immutable-frame rejection, asset success/failure, transactional fallback-to-rig success and rollback, candidate-preparation rollback, committed-refresh retirement-failure injection for action stop, `stopAllAction` and `uncacheRoot`, persistent teardown-failure regressions for both current and deferred animation sets, adapter-level failed-view retention and retry, repeated-reset single-action ownership, stale character and animation completion after reset/teardown, shared-template disposal ordering, reconciliation/reset/teardown, generated-source ownership guards, production browser smoke, full CI and Vercel evidence on one frozen head. No validation may depend on gameplay mutation or visual tuning.
