...
(() => {
  ...
  function releaseBall(player, dx, dy, speed, type, feedback = {}) {
    ...
    publishCompatibilityEvent(GameEventType.BALL_KICKED, createCompatibilityKickEventPayload({
      commandType,
  ...
  function passBall(player, charge=.35, oneTwo=false) {
    ...
    const d = distance(player, target); const power=lerp(.82,1.16,clamp(charge,.08,1));releaseBall(player, leadX - player.x, leadY - player.y, clamp((430 + d * .35)*power, 440, 760), "pass", { commandType: GameCommandType.SHORT_PASS, power: clamp(charge, 0, 1), style: oneTwo ? "one-two" : "short", targetId: compatibilityPlayerId(target) });
  ...
  function loftBall(player, charge=.45) {
    ...
    if(!target)return;const power=lerp(.82,1.18,clamp(charge,.08,1));releaseBall(player,target.x+target.vx*.32-player.x,target.y+target.vy*.32-player.y,clamp((610+distance(player,target)*.3)*power,580,940),"loft", { commandType: GameCommandType.LOFTED_PASS, power: clamp(charge, 0, 1), style: "loft", targetId: compatibilityPlayerId(target) });ball.vz*=lerp(.82,1.14,charge);
  ...
  function updateUI(snapshot, scoringTeam = null) {
    ...
    ui.gameClock.textContent = hud.clock;
    ui.homeScore.textContent = hud.score[HOME]; ui.awayScore.textContent = hud.score[AWAY];
    const player = hud.selectedPlayer; if (player) {
      const selectedKey=player.id;
    ...
    const onboardingElapsed=hud.elapsed;const hintsActive=Boolean(input.actionStart||input.magnitude>.05||hud.state!=="playing");
    ...
    ui.possessionStat.textContent = `${hud.homePossession}%`; ui.possessionBar.style.width = `${hud.homePossession}%`;
    ui.homeShots.textContent = hud.shots[HOME]; ui.awayShots.textContent = hud.shots[AWAY];
    ui.passStat.textContent = `${hud.passAccuracy}%`;
