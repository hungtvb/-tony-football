...
function adapter() {
  return createSnapshotCameraReplayAdapter({ worldWidth: 1200, worldHeight: 700, viewportWidth: 1200, viewportHeight: 700, cameraConfig: cameraHudConfig.camera, sampleRate: 10, maxFrames: 8 });
}
...
test("historical replay visuals preserve current engine-owned metadata", () => {
  const owner = adapter(); owner.attach();
  const historical = snapshot(1, { elapsed: .1, ballX: 321, score: [0, 0], time: 149.9 });
  const beforeReplay = snapshot(2, { elapsed: .2, ballX: 421, score: [0, 0], time: 149.8 });
  owner.project(frame(historical)); owner.project(frame(beforeReplay, historical));
  const current = snapshot(3, { elapsed: .2, replayActive: true, replayElapsed: 0, replayDuration: 2, ballX: 999, score: [1, 0], time: 147.25, goalSequence: { phase: "replay", scorerId: "home-0" } });
  const projection = owner.project(frame(current, beforeReplay));
  assert.equal(projection.renderSnapshot.match, current.match);
  assert.deepEqual(projection.renderSnapshot.match.score, [1, 0]);
  assert.equal(projection.renderSnapshot.match.time, 147.25);
  assert.equal(projection.renderSnapshot.match.replay.active, true);
  assert.equal(projection.renderSnapshot.ball.x, historical.ball.x);
});

test("compatibility replay facade cannot activate or advance replay", () => {
  const owner = adapter(); owner.attach();
  assert.equal("start" in owner.replay, false);
  assert.equal("loadFrames" in owner.replay, false);
  assert.equal("syncElapsed" in owner.replay, false);
  owner.project(frame(snapshot(1)));
  assert.equal(owner.replay.active, false);
  assert.equal(owner.replay.update(500), false);
  assert.equal(owner.replay.active, false);
});
...
test("fresh kickoff restoration clears prior replay history", () => {
  ...
  assert.equal(projection.replay.active, false); assert.equal(projection.renderSnapshot, kickoff); assert.equal(owner.diagnostics().replay.historyFrames, 1);
});
...
test("reset and teardown are explicit and terminal", () => {
  const owner = adapter(); assert.equal(owner.attach(), true); owner.project(frame(snapshot(1))); assert.equal(owner.reset(), true); assert.equal(owner.diagnostics().renderCount, 0); assert.equal(owner.teardown(), true); assert.equal(owner.diagnostics().status, "disposed"); assert.equal(owner.project(frame(snapshot(2))), null); assert.equal(owner.attach(), false);
});
