...
test("generated compatibility runtime no longer owns or diagnoses the match Canvas renderer", () => {
  prepareTon80Game();
  const text = generated();
  const forbidden = ["renderFallback2D", "drawFallbackPlayerDetail", "canvas.getContext", "function drawPitch(", "function drawPlayer(", "function drawBall(", "function drawEffects(", "function drawScreenEffects(", "__TONY_CANVAS_MATCH_BRIDGE__"];
  for (const token of forbidden) assert.equal(text.includes(token), false, `generated/game.js must not contain ${token}`);
  assert.equal(text.includes("__TONY_COMPATIBILITY_PRESENTATION_PORT__"), true);
});
