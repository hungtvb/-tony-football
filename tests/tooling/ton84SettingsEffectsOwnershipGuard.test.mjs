...
test("TON-84 generated runtime consumes the single settings/effects bridge", async () => {
  const [generated, entry, settings, effects] = await Promise.all([
  ...
  for (const contract of ["window.__TONY_SETTINGS_EFFECTS_BRIDGE__", "settingsEffectsBridge.settings.configure", "settingsEffectsBridge.effects.projectTrail", "settingsEffectsBridge.effects.projectCharge", "settingsEffectsBridge.effects.snapshot"]) assert.ok(generated.includes(contract), `missing generated contract ${contract}`);
