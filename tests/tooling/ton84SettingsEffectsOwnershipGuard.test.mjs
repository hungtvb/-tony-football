...
test("TON-85 final runtime consumes the outward-only presentation port", async () => {
  ...
  for (const contract of ["window.__TONY_COMPATIBILITY_PRESENTATION_PORT__", "presentationPort.configureSettings", "presentationPort.recordBallTrail", "presentationPort.stepEffects"]) assert.ok(generated.includes(contract), `missing generated contract ${contract}`);
  ...
  for (const contract of ["createBrowserSettingsAdapter", "createBrowserEffectsAdapter", "createBrowserEffectsViewAdapter", "__TONY_COMPATIBILITY_PRESENTATION_PORT__", "FO4_CONTROLS"]) assert.ok(entry.includes(contract), `entry missing ${contract}`);
  ...
  assert.ok(ballView.includes("activeCharge.color")); assert.ok(canvasRenderer.includes("effects?.charge"));
});
